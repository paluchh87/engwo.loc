...
class DB
{
    ...
    public static function getDbh()
    {
        if (!self::$dbh) {
            try {
                ...
                self::$dbh->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);//ERRMODE_WARNING
    ...
    public static function getAllWithBindValue($query, $param = [])
    {
        self::$sth = self::getDbh()->prepare($query);
        if (!empty($param)) {
            foreach ($param as $key => $val) {
                ...
                self::$sth->bindValue(':' . $key, $val, $type);
            }
        }
        self::$sth->execute();
